"""Generate a LaTeX ablation table for few-shot exemplar selection strategy.

Rows: selection strategy (Random, Balanced, Similarity) with num_shots=5 fixed.
...
# Ordered list of selection strategies (determines row order)
SELECTION_STRATEGIES = ["random", "balanced", "similarity"]

SELECTION_DISPLAY_NAMES: dict[str, str] = {
    "random": "Random",
    "balanced": "Balanced",
...
SELECTION_RUNS: dict[str, tuple[str, str]] = {
    ...
    "TODO_internvl_balanced": ("InternVL3.5-8B", "balanced"),
    "TODO_internvl_similarity": ("InternVL3.5-8B", "similarity"),
    "TODO_qwen_random": ("Qwen3-VL-8B", "random"),
    "TODO_qwen_balanced": ("Qwen3-VL-8B", "balanced"),
    "TODO_qwen_similarity": ("Qwen3-VL-8B", "similarity"),
}
...
def compute_col_bests(
    ...
    """Compute per-column best values across all selection strategies per model."""
...
def generate_latex(data: dict[tuple[str, str], list[float | None]]) -> str:
    ...
    for selection in SELECTION_STRATEGIES:
        display = SELECTION_DISPLAY_NAMES[selection]
...
{rows_str}
        ...
        \\bottomrule
    \\end{{tabular}}}}
    \\caption{{\\textbf{{Few-Shot Selection Ablation.}} Effect of exemplar selection strategy on fall detection performance with 5 in-context shots (ascending order, with delimiters). Best results per model are \\textbf{{bolded}}.}}
