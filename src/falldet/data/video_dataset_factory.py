...
def get_video_datasets(
    ...
    split="cs",
    **processing_kwargs,  # size, max_size, seed, etc.
    ...
    for ds_item in dataset_config.video_datasets:
        ...
        dataset_split = ds_item.split
        split_root_config = None

        # Extract split from split_root if present (this is the source of truth)
        if "config=" in ds_item.split_root:
            split_root_config = ds_item.split_root.split("config=")[-1].split("/")[0]
        ...
        if dataset_split is None:
            if split_root_config is not None:
                ...
                logger.info(f"Using split '{dataset_split}' from split_root for {ds_item.name}")
            else:
                dataset_split = split
                logger.info(
                    f"Using split '{dataset_split}' from function parameter for {ds_item.name}"
                )
        else:
            logger.info(
                f"Using explicit split '{dataset_split}' from dataset config for {ds_item.name}"
            )
        ...
        if split_root_config is not None and dataset_split != split_root_config:
            error_msg = (
                f"\n{'=' * 80}\n"
                f"CRITICAL CONFIGURATION ERROR: Split mismatch for dataset '{ds_item.name}'\n"
                f"{'=' * 80}\n"
                f"  Dataset split config: '{dataset_split}'\n"
                f"  Split from split_root: '{split_root_config}'\n"
                f"  Split root path: {ds_item.split_root}\n"
                f"\n"
        ...
        disk_cache = (
            TensorDiskCache(
                ...
                read_only=config.data.cache_read_only,
            )
        ...
        if len(dataset) > 0:
            ...
            dataset_key = f"{ds_item.name}_{split_suffix}"
            ...
            group = ds_item.evaluation_group
            ...
            if run is not None and run.id:
                run.log({f"{ds_item.name}_{mode}_size": len(dataset)})
        else:
            logger.warning(f"Dataset {ds_item.name} is empty for {mode} split. Skipping.")
            if run is not None and run.id:
                run.log({f"{ds_item.name}_{mode}_size": 0})
    ...
    if return_individual:
        ...
        if dataset_config.create_all_combined:
