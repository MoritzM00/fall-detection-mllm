...
def test_pretrained_base_uses_fresh_lora_policy_against_base_reference(tmp_path):
    torch.manual_seed(0)
    base = GPT2LMHeadModel(
        GPT2Config(
            vocab_size=7,
    ...
    trainer = VideoDPOTrainer(
        model=base,
        ref_model=None,
        peft_config=LoraConfig(r=2, target_modules=["c_attn"], bias="none"),
        args=DPOConfig(
            output_dir=str(tmp_path),
            report_to="none",
            remove_unused_columns=False,
    ...
    assert trainer.ref_model is None
    assert set(trainer.model.peft_config) == {"default"}
    assert all(
